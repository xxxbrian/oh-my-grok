...
fn main() {
    ...
    let runtime =
        xai_tty_utils::runtime::build_with_blocking_pool(&mut builder).unwrap_or_else(|e| {
            eprintln!("omg: failed to start tokio runtime: {e}");
...
async fn run_update_command(
    ...
    let telemetry_cfg = xai_grok_shell::config::load_agent_config_disk_only()
        .map_err(|e| tracing::warn!("omg update: telemetry init skipped (agent config: {e})"))
        .ok();
...
mod tests {
    ...
    async fn fork_update_command_stops_before_the_upstream_updater() {
        ...
        let trigger = auto_update::CliUpdateTrigger::UserCommand;
        run_update_command(false, false, false, None, None, trigger, &update_config)
            .await
            .unwrap();
        run_update_command(true, false, false, None, None, trigger, &update_config)
            .await
            .unwrap();
        run_update_command(true, true, false, None, None, trigger, &update_config)
            .await
            .unwrap();
    }
