...
```bash
omg -p "Run the tests" --always-approve
...
```bash
omg --always-approve -p "Run the test suite"
omg --permission-mode auto
grok agent --always-approve serve --bind 127.0.0.1:2419 --secret <token>
```
...
```bash
omg -p "Deploy the service" --always-approve --deny 'Bash(rm -rf *)'
```
