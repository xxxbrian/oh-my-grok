...
Like `streaming-json`, this stream is read only. Tool approvals and other bidirectional flows use the ACP interface (`omg agent`).
...
```bash
omg -p "Format all files" --always-approve
omg -p "Run the tests and fix any failures" --cwd ~/projects/my-app --always-approve
```
