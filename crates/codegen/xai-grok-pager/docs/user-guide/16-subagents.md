...
Oh My Grok discovers file-based personas from these locations, in priority order:
...
> **Note:** Oh My Grok applies personas through subagent resolution and roles, not through a `spawn_subagent` parameter. The main agent does not pass a persona name when it spawns a child.
...
When a persona applies, Oh My Grok resolves the effective model and reasoning effort in this order, highest priority first:
...
Oh My Grok manages worktrees through the `x.ai/git/worktree/*` extension methods, including an apply operation that merges changes back into the main working directory.
...
Oh My Grok also discovers roles from `.grok/roles/*.toml` and personas from `.grok/personas/*.toml`. Inline `config.toml` definitions take precedence over files.

---

## The Tasks Pane (TUI)

Oh My Grok shows running and finished work in side panes on the agent screen:
