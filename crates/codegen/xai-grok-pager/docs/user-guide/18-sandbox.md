...
- `omg workspace start`, `restart`, and `resume` are unavailable; `pause`, `stop`, and `status` still work
...
`omg --continue`, or `omg -r`), Grok restores that same profile automatically —
