...
impl TitleManager {
    ...
    pub fn update(&mut self, state: &TitleState<'_>) -> Option<String> {
        ...
        if !has_parts {
            self.composed.clear();
            self.composed.push_str(xai_grok_config::CLI_NAME);
        }
    ...
    pub fn reset(&mut self) -> String {
        let esc = build_title_escape(xai_grok_config::CLI_NAME);
        self.last_title.clear();
        self.last_title.push_str(xai_grok_config::CLI_NAME);
...
fn write_item(
    ...
    match item {
        TitleItem::Grok => {
            ...
            buf.push_str(xai_grok_config::CLI_NAME);
        }
        TitleItem::Spinner => {
...
mod tests {
    ...
    fn session_name_and_grok_joined_with_separator() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "my project - omg");
    }
    ...
    fn spinner_only_shown_when_active() {
        ...
        mgr.update(&state);
        assert!(mgr.last_title.contains(" - omg"));
    ...
    fn activity_shows_waiting_when_busy_without_activity() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "Waiting - omg");
    }
    ...
    fn activity_prefers_specific_activity_over_waiting() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "Thinking - omg");
    }
    ...
    fn model_item_shown_when_present() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "grok-3 - omg");
    }
    ...
    fn turn_timer_shown_when_above_one_second() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "42s - omg");
    }
    ...
    fn default_config_active_turn_with_permissions() {
        ...
        for t in [&t1, &t2] {
            assert!(t.contains("omg"), "title missing 'omg': {t}");
    ...
    fn all_items_present_in_order() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "Thinking - proj - grok-3 - workspace - omg");
    }
