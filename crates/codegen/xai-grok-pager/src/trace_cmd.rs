...
async fn run_upload(
    ...
    let upload_method = match upload_method {
        ...
        None => {
            ...
            anyhow::bail!(
                "No upload credentials. Run `omg login` or set a deployment key. \
...
impl UploadAttempt<'_> {
    ...
    pub fn handle_failure(&self, error: &anyhow::Error) -> anyhow::Error {
        ...
        if self.json {
            ...
            eprintln!("  Retry:  omg trace {}", self.session_id);
    ...
    fn write_debug_log(&self, error: &anyhow::Error, output_dir: &Path) -> PathBuf {
        ...
        let _ = writeln!(log, "Oh My Grok version: {}", xai_grok_version::OMG_VERSION);
