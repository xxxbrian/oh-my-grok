...
pub(super) fn render(failure: &StartupFailure) -> String {
    ...
    let mut report = match &failure.reason {
        Reason::TimedOut { waited, timings } => {
            ...
            format!(
                "Couldn't start Oh My Grok: startup timed out after {}.\n\n{explanation}",
...
impl Advice {
    fn explanation(&self) -> String {
        ...
        if let Some(earlier) = self.earlier {
            ...
            let _ = write!(
                explanation,
                " Oh My Grok spent the first {} on the {target}.",
...
impl NextStep {
    fn text(self) -> &'static str {
        match self {
            Self::Retry => "Start Oh My Grok again.",
            Self::CheckNetworkThenRetry => {
                "Check your network connection, then start Oh My Grok again."
            }
            Self::RestartSharedLeader => {
                "Stop it with the command below, which also stops any other Oh My Grok \
                 session using it, then start Oh My Grok again."
            }
        }
    }
    ...
    fn command(self) -> Option<&'static str> {
        match self {
            ...
            Self::RestartSharedLeader => Some("omg leader kill"),
        }
    }
}
