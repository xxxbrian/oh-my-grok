...
fn canonical_and_short_ids_resolve_to_canonical_id() {
    ...
    assert_eq!(command, "omg doctor fix ssh-wrap");
    assert_eq!(
        resolve_fix_id(command.strip_prefix("omg doctor fix ").unwrap()).unwrap(),
        SSH_WRAP_ID
    );
...
fn bash_zsh_and_fish_plans_use_exact_paths_and_aliases() {
    ...
    for (shell, relative, alias) in [
        ("/bin/bash", ".bashrc", "alias ssh='omg wrap ssh'"),
        ("/bin/zsh", ".zshrc", "alias ssh='omg wrap ssh'"),
        (
            "/usr/local/bin/fish",
...
fn shell_aliases_expand_to_exact_argv_and_bypass_is_explicit() {
    ...
    .unwrap();
    use std::os::unix::fs::PermissionsExt as _;
    std::fs::set_permissions(&omg, std::fs::Permissions::from_mode(0o755)).unwrap();

    if let Some(bash) = find_on_path("bash") {
        let rc = temp.path().join("bashrc");
        std::fs::write(&rc, "alias ssh='omg wrap ssh'\n").unwrap();
        let command = format!(
    ...
    let mut shell = std::process::Command::new(bash);
    shell
        .args(["-ic", "alias ssh='omg wrap ssh'; command ssh host"])
        .env("CAPTURE", &capture)
        .env(
            "PATH",
            format!(
