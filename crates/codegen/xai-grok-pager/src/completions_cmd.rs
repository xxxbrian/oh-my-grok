//! `omg completions <shell>` — generate shell completion scripts.
//!
...
/// `$line[2]` empty, and the dispatch falls through — so `omg worktree <TAB>`
...
fn fix_zsh_root_prompt_positional(script: &str) -> String {
    ...
    out = out.replacen(
        &format!(r#"curcontext="${{curcontext%:*:*}}:{CLI_NAME}-command-$line[2]:""#),
        &format!(r#"curcontext="${{curcontext%:*:*}}:{CLI_NAME}-command-$line[1]:""#),
        1,
    );
    out = out.replacen(r#"case $line[2] in"#, r#"case $line[1] in"#, 1);
    out
}
...
mod tests {
    ...
    // so `omg worktree <TAB>` re-offered every top-level command (upstream
    // clap-rs/clap#6282).
    #[test]
